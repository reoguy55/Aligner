...
var GridBuilder = (function () {
    ...
    function onPathUpdate() {
        ...
        var startIdx = -1;
        if (mAlignType === 'Global') {
          startIdx = 0;
        } else if (mAlignType === 'Fitting') {
          var gaps = 0;
          while (alignedSideSeq[gaps] === mGapSymbol) {
            gaps += 1;
          }
          startIdx = gaps;
        }
        ...
        for (var idxSide in alignedSideSeq) {
          if (alignedSideSeq[idxSide] === mGapSymbol) {
            continue;
          }
    ...
    mSelf = {
        ...
        highlightOptimal: function(alignType) {

            mIsCustomPathMode = false;
            var width = mTopSequence.length + 1;
            var height = mSideSequence.length + 1;
            var bestFitX = -1;
            var maxFitScore = Number.NEGATIVE_INFINITY;
            // console.log(mCellMap);
            for (var i = 0; i < width; i++) {
                // console.log(mCellMap[i + '_' + height - 1]['winningScore']);
                var currentCell = mCellMap[i + '_' + (height - 1)];
                if (currentCell.winningScore > maxFitScore) {
                  maxFitScore = currentCell.winningScore;
                  bestFitX = i;
                }
            }
            ...
            var bestLocX = -1;
            var bestLocY = -1;
            var maxLocScore = Number.NEGATIVE_INFINITY;
            for (var i = 0; i < width; i++) {
              for (var j = 0; j < height; j++) {
                var currentCell = mCellMap[i + '_' + j];
                if (currentCell.winningScore > maxLocScore) {
                  maxLocScore = currentCell.winningScore;
                  bestLocX = i;
                  bestLocY = j;
                }
              }
            ...
            while (currentX > -1 && currentY > -1) {
                ...
                currentDom.click();
                if (mAlignType === 'Fitting' && currentY <= 0) {
                  break;
                } else if (mAlignType === 'Local' && currentCell.winningScore == 0) {
                  break;
                }
                var direction = null;
        ...
        rebuildGlobal: function(domContainer, resultContainer, matchScore, mismatchScore, gapScore, seqSide, seqTop) {
        ...
        rebuildFitting: function(domContainer, resultContainer, matchScore, mismatchScore, gapScore, seqSide, seqTop) {
          seqTop = seqTop.toUpperCase();
          seqSide = seqSide.toUpperCase();
          mCurrentPath = [];
          mDomContainer = domContainer;
        ...
        rebuildLocal: function(domContainer, resultContainer, matchScore, mismatchScore, gapScore, seqSide, seqTop) {
          seqTop = seqTop.toUpperCase();
          seqSide = seqSide.toUpperCase();
          mCurrentPath = [];
          mDomContainer = domContainer;
        ...
        rebuildTable: function(domContainer, resultContainer, matchScore, mismatchScore, gapScore, seqSide, seqTop, alignType) {
            ...
            if (alignType === "Global") {
              GridBuilder.rebuildGlobal(domContainer, resultContainer, matchScore, mismatchScore, gapScore, seqSide, seqTop);
            } else if (alignType === "Fitting") {
              GridBuilder.rebuildFitting(domContainer, resultContainer, matchScore, mismatchScore, gapScore, seqSide, seqTop);
            } else if (alignType === "Local") {
              GridBuilder.rebuildLocal(domContainer, resultContainer, matchScore, mismatchScore, gapScore, seqSide, seqTop);
            } else {
              console.log("No alignment selected");
              return;
            }
